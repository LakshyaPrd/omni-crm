...
import {
  ...
  X, ChevronDown, ArrowUpDown, LayoutList, LayoutGrid, Brain,
  Users, CheckSquare, Table2,
} from "lucide-react";
      ...
      <div className="animate-slide-in-up">
        <div className="flex items-center justify-between mb-5">
          <div>
            <div className="flex items-center gap-2 mb-0.5">
              ...
              <span className="bg-brand-100 text-brand-700 text-xs font-semibold px-2 py-0.5 rounded-full">{importedLeads.length} leads</span>
            </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <Button><Plus className="w-4 h-4" />Import</Button>
          </div>
        </div>
        ...
        <div className="mb-3">
          <div className="relative flex items-center bg-white border border-slate-200 rounded-xl shadow-card hover:border-slate-300 transition-all focus-within:border-brand-400 focus-within:ring-2 focus-within:ring-brand-200">
            <svg className="ml-3.5 w-4.5 h-4.5 text-slate-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
            <input
              className="flex-1 px-3 py-3 text-sm text-slate-900 placeholder-slate-400 bg-transparent focus:outline-none"
              placeholder='Search by name, title, company, location... (Boolean: "VP AND SaaS NOT intern")'
              value={search}
              onChange={(e) => setSearch(e.target.value)}
            />
            {search && (
              <button onClick={() => setSearch("")} className="p-1.5 hover:bg-slate-100 rounded-lg text-slate-400 mr-1"><X className="w-3.5 h-3.5" /></button>
            )}
            <span className="text-xs text-slate-500 px-2 py-0.5 bg-slate-100 rounded-full font-medium mr-2 whitespace-nowrap">{filtered.length} results</span>
            <button className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-medium text-slate-500 hover:bg-slate-100 mr-2">
              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"/></svg>
              Save search
            </button>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap mb-4">
          ...
          {activeChips.map((chip) => (
            <div key={chip} className="flex items-center gap-1 bg-brand-50 text-brand-700 text-xs font-medium px-2.5 py-1 rounded-full border border-brand-200">
              {chip}<button onClick={() => setActiveChips((p) => p.filter((c) => c !== chip))} className="ml-0.5"><X className="w-3 h-3" /></button>
            </div>
          ))}
          {(["linkedin","apollo","salesrobot","smartlead"] as const).map((src) => (
            <div key={src}><SourceBadge source={src} size="sm" /></div>
          ))}
          {activeChips.length > 0 && <button onClick={() => setActiveChips([])} className="text-xs text-red-500 hover:text-red-700 ml-1 font-medium">Clear all</button>}
          <div className="ml-auto flex items-center gap-2">
                ...
                <div className="absolute right-0 top-full mt-1 w-44 bg-white border border-slate-100 rounded-xl shadow-card-lg py-1 z-20">
                  {sortOptions.map((o) => (
                    <button key={o} onClick={() => { setSort(o); setShowSortMenu(false); }}
                      className={cn("w-full text-left px-3 py-2 text-xs hover:bg-slate-50", sort === o ? "text-brand-600 font-semibold" : "text-slate-700")}>{o}
                    </button>
                  ))}
                </div>
              )}
            ...
            <div className="flex border border-slate-200 rounded-lg overflow-hidden">
              {([{v:"list" as const,I:LayoutList},{v:"grid" as const,I:LayoutGrid},{v:"table" as const,I:Table2}]).map(({v,I}) => (
                <button key={v} onClick={() => setView(v)} className={cn("p-1.5 transition-colors", view===v?"bg-brand-600 text-white":"bg-white text-slate-500 hover:bg-slate-50")}><I className="w-4 h-4" /></button>
              ))}
            </div>
          </div>
        </div>
          ...
          <div className="bg-brand-600 text-white rounded-xl p-3 mb-4 flex items-center gap-3 animate-slide-in-up">
            ...
            <div className="flex items-center gap-1.5 ml-2 flex-wrap">
              {([{I:Mail,l:"Email"},{I:Tag,l:"Tag"},{I:Users,l:"Assign"},{I:Plus,l:"Campaign"},{I:Download,l:"Export"},{I:Trash2,l:"Delete"}]).map(({I,l}) => (
                <button key={l} className="flex items-center gap-1.5 px-3 py-1.5 bg-white/20 hover:bg-white/30 rounded-lg text-xs font-medium transition-colors"><I className="w-3.5 h-3.5" />{l}</button>
              ))}
            </div>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm text-slate-600">
                Showing <span className="font-semibold text-slate-900">{filtered.length}</span> of <span className="font-semibold text-slate-900">{importedLeads.length}</span> leads
              </span>
            ...
            {view !== "table" && (
              <div className="flex items-center justify-between mt-5 pt-4 border-t border-slate-100">
                <span className="text-xs text-slate-500">Showing {filtered.length} of {importedLeads.length}</span>
                <div className="flex items-center gap-1">
                  {[1,2,3,"...",12].map((p,i) => (
                    <button key={i} className={`w-7 h-7 text-xs rounded-lg transition-colors ${p===1?"bg-brand-600 text-white":"text-slate-600 hover:bg-slate-100"}`}>{p}</button>
                  ))}
